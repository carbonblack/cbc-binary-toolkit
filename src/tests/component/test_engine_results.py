...
def cbcloud_api_mock(monkeypatch, cbc_cloud_api):
    """Mock CBCloudAPI for unit tests."""
...
def test_init(state_manager, cbc_cloud_api, engine_results):
    ...
    assert engine_results.cbc_api == cbc_cloud_api
...
def test_store_ioc(cbcloud_api_mock, state_manager, engine_results, input):
    """Test _store_ioc"""
    for ioc in input:
...
def test_store_ioc_invalid(cbcloud_api_mock, state_manager, engine_results, input):
...
def test_send_reports(cbcloud_api_mock, state_manager, engine_results, input):
    ...
    assert cbcloud_api_mock._last_request_data["description"] == "Automated report generated by Binary Analysis SDK"

    # Check that all IOCs are attached to a report
    for ioc in input_with_severity:
        SENT = False
        log.error(f"All request data: {cbcloud_api_mock._all_request_data}")
        for report in cbcloud_api_mock._all_request_data:
...
def test_send_reports_invalid(cbcloud_api_mock, state_manager, engine_results, input):
...
def test_send_reports_404(cbcloud_api_mock, state_manager, engine_results, input):
