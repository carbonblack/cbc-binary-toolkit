...
class EngineResults:
    ...
    def reload(self, feed_id):
        """Load unsent reports from the database and send them"""
        try:
            for severity in range(SEVERITY_RANGE):
                ...
                for ioc in iocs:
                    ...
                    report_meta = {
                        "id": str(uuid.uuid4()),
                        "timestamp": int(now),
                        "title": f"{self.engine_name} Severity {severity + 1} - "
                                 f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(now))}",
                        "description": "Automated report generated by Binary Analysis SDK",
                        "severity": severity + 1,
                        "iocs_v2": [ioc]
                    }
                    report_to_send = Report(self.cbth, initial_data=report_meta, feed_id=feed_id)
                    report_to_send.update()
                    log.info(f"Report ({report_meta['title']}) sent to feed {feed_id}")
                    # Clear report items from the database
                    self.state_manager.clear_report_items(severity + 1, self.engine_name)
            log.info("Reports successfully sent from database reload")
            return True
        except (Exception, ObjectNotFoundError) as e:
            log.error(f"Error while sending reports to feed {feed_id}: {e}")
            return False
